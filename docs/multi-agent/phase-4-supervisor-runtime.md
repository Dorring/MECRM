...
> **R4 Revision** — This document reflects the R4 audit fixes (commit `<TBD>`).
...
> request-changes). R4 addresses 4 P0 and 2 P1 issues from the R3 review,
> focused on **runtime identity, Usage trust, and budget data integrity**:
> Run Identity Probe must execute Plan Conflict semantics before any other
> Pre-flight check; Usage Trust must be bound to the Invoker's capabilities
> rather than self-reported by the Receipt; invalid Receipts must still
> charge observed tool calls to the budget; Agent Result Version must match
> the ExecutionBinding snapshot; Agent Call Budget must be deterministically
> pre-allocated in `task_id` order before any coroutine is created.
...
### R4 P1-1: Deterministic Pre-Dispatch (Agent Call Budget)
...
R4 在 `on_wave_started` **之后**、协程创建**之前**插入同步 `pre_dispatch` 过滤器：

```python
PreDispatch = Callable[[list[AgentTask]], list[AgentTask]]
```

`SupervisorRuntime._build_pre_dispatch(accountant)` 返回的过滤器：

1. 接收**已按 `task_id` 升序排序**的 ready 列表
2. 用 `has_agent_call_budget()`（side-effect-free 检查）判断是否还有剩余 slot
3. 通过 `reserve_agent_call()` 按顺序预留
4. 返回允许执行的子集；不在子集中的 Task 直接被 Scheduler 标记 `skipped`（`skip_reason="agent_call budget exhausted"`），**不创建协程**
...
**保证**：同 Plan + 同 Budget 在任何事件循环实现下，哪些 Task 获得 agent call slot 是确定的（按 `task_id` 升序）。Retry attempt（`attempt_idx > 0`）仍在 `_execute_task` 内部直接调用 `reserve_agent_call()`——pre-dispatch 只负责**首 attempt** 的预算分配。
...
### R4 P0-2: Invoker-bound Usage Trust

R3 的 `TrustedUsageInvoker` 标记 Protocol 是**可伪造**的——任意自定义 Invoker 可以直接在 Receipt 中设置 `usage_trust="trusted_adapter"` 而无需实际实现 `usage_is_verified=True`。生产代码从未读取 `usage_is_verified` 属性。同样，`verified_provider` 只要求 `provider_metadata is not None`，但 `provider_metadata` 本身也是 Handler 返回的普通数据，不是不可伪造的 Provider Usage Receipt。

R4 用 **`UsageVerificationCapabilities`** 替代可伪造的 marker Protocol：

```python
class UsageVerificationCapabilities(StrictContract):
    model_config = ConfigDict(frozen=True, extra="forbid")

    verifies_tokens: bool = False  # 是否能验证 token usage
    verifies_cost: bool = False    # 是否能验证 cost usage
    source_id: str                 # 稳定标识，用于诊断
```

Invoker 通过 `usage_capabilities` property 暴露其能力：
...
`get_usage_capabilities(invoker)` 使用 `getattr`（不是 `isinstance`）读取该 property，未实现的 Invoker 默认返回 `_UNVERIFIED_CAPABILITIES`（`verifies_tokens=False, verifies_cost=False, source_id="unverified"`）——避免破坏现有测试 fake。

**强制规则**（在 `_BudgetAccountant.record_receipt()` 中执行）：

| 配置 | Invoker Capability 要求 | 接受的 `usage_trust` |
|---|---|---|
| 未设置 Token/Cost Budget | 任何 | 任何（含 `UNVERIFIED`） |
...
`record_receipt()` 现在接受必需的 `invoker_capabilities` 参数：
...
`RegistryAgentInvoker` 默认 `usage_trust=VERIFIED_PROVIDER` + `verifies_tokens=True, verifies_cost=True`（因为 Receipt 的 token/cost 来自 `AgentResult.provider_metadata`，由 LLM Provider 原始返回）。`DeterministicFakeInvoker` 默认 `UNVERIFIED` + 完全无 capability——除非显式传入 `usage_capabilities`。
...
# R4 P1-1: Deterministic Pre-Dispatch
PreDispatch = Callable[[list[AgentTask]], list[AgentTask]]
```
